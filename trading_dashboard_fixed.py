...
# Streamlit Cloud pe /tmp use karo — writable guaranteed hai
# Note: /tmp ephemeral hai — app restart pe clear hoga
_IS_CLOUD = os.environ.get("HOME", "").startswith("/home/appuser") or \
            os.path.exists("/home/appuser")
OI_HISTORY_DIR = "/tmp/oi_history" if _IS_CLOUD else \
    os.path.join(os.path.dirname(os.path.abspath(__file__)), "oi_history")
...
def _get_secret(key, default=""):
    """Streamlit Secrets aur os.environ dono se try karo"""
    try:
        import streamlit as _st
        val = _st.secrets.get(key, None)
        if val:
            return val
    except Exception:
        pass
...
API_KEY    = _get_secret("UPSTOX_API_KEY")
SECRET_KEY = _get_secret("UPSTOX_SECRET_KEY")
TOKEN_FILE = "/tmp/upstox_token.json" if _IS_CLOUD else "upstox_token.json"
...
TELEGRAM_FILE = "/tmp/telegram_config.json" if _IS_CLOUD else "telegram_config.json"
...
# ── Notification helper ───────────────────────────────────────
NOTIF_FILE = "/tmp/notifications.json" if _IS_CLOUD else "notifications.json"
...
def add_notification(ntype, title, msg):
    ...
    # NOTE: st.markdown removed here — calling st.* from non-main thread causes RuntimeError
...
def save_token(token):
    try:
        ...
        print(f"[WARN] save_token failed: {e}")
...
def get_access_token(auth_code):
    REDIRECT_URI = _get_secret(
        "UPSTOX_REDIRECT_URI",
        "https://trading-dashboard-eqcqbcuwrwfvovcmrsyqpp.streamlit.app"
    )
    try:
        resp = requests.post(
...
def get_fii_dii_data():
    ...
    try:
        ...
        ts = int(time.time() * 1000)
        resp = session.get(f"https://www.nseindia.com/api/fiidiiTradeReact?_={ts}", headers=headers, timeout=8)
        if resp.status_code == 200:
...
with col_terminal:
    ...
    if terminal_path.exists():
        # Serve via Streamlit static files if available, else show info
        st.markdown(
            '<div style="background:#f59e0b20;border:1px solid #f59e0b60;border-radius:6px;'
            'padding:5px 12px;font-size:12px;color:#f59e0b;text-align:center;font-weight:bold;line-height:2;">'
            '🖥️ Live Terminal (open manually)</div>',
            unsafe_allow_html=True
        )
    else:
        st.markdown(
            '<div style="background:#33333320;border:1px solid #55555560;border-radius:6px;'
            'padding:5px 12px;font-size:12px;color:#888;text-align:center;line-height:2;">'
            '🖥️ Terminal file not found</div>',
            unsafe_allow_html=True
        )
