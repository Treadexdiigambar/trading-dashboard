    ...
    with tab:
        ...
        if not df.empty and result["atm_strike"]:
            ...
            if atm_idx_list:
                ...
                # Bar colors — top strikes bright, baaki dim (rgba format)
                call_colors = ["rgba(255,34,34,1.0)" if s in top3_call else "rgba(255,82,82,0.3)" for s in df_d["Strike"].tolist()]
                put_colors  = ["rgba(0,255,136,1.0)" if s in top3_put  else "rgba(0,230,118,0.3)" for s in df_d["Strike"].tolist()]
